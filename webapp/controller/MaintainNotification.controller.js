...
sap.ui.define(["i2d/eam/pmnotification/create/s1/util/Constants", "i2d/eam/pmnotification/create/s1/controller/BaseController", "sap/ui/model/json/JSONModel", "i2d/eam/pmnotification/create/s1/util/TextTemplateHandler", "sap/ui/core/format/DateFormat", "i2d/eam/pmnotification/create/s1/model/formatter", "sap/m/MessageBox",], function (C, B, J, T, D, f, M) {
  ...
  return B.extend("i2d.eam.pmnotification.create.s1.controller.MaintainNotification", {
    ...
    initViewPropertiesModel: function () {
      ...
      if (s) {
        this.getView().getParent().setModel(this._oViewProperties, C.MODEL.VIEW_PROP.NAME);
      }
    ...
    refreshViewProperties: function () {
      ...
      if (typeof this._oAppModel.getProperty(C.MODEL.APP_MODEL.PROPERTIES.IS_CREATE_MODE) === "undefined") {
        this._oViewProperties.setProperty(C.MODEL.VIEW_PROP.PROPERTIES.GENERAL.CHANGE_ALLOWED, false);
      } else if (this._oAppModel.getProperty(C.MODEL.APP_MODEL.PROPERTIES.IS_CREATE_MODE)) {
        this._oViewProperties.setProperty(C.MODEL.VIEW_PROP.PROPERTIES.GENERAL.CHANGE_ALLOWED, !this.hasPendingChangesForOtherNotification({
      ...
      this._oViewProperties.setProperty(g.ADDITIONAL_SEARCH_OPTIONS_ON, this.isLinkAdditionalSearchOptionsVisible());
      this._oViewProperties.setProperty(C.MODEL.VIEW_PROP.PROPERTIES.GENERAL.CNT_CURRENT_NOTIFICATIONS, 0);
    },
    initializeNewNotificationData: function () {
      var a;
      var n;
      this._oAppModel.setProperty(C.MODEL.APP_MODEL.PROPERTIES.IS_CREATE_MODE, true);
      if (!this._oAppModel.getProperty(C.MODEL.APP_MODEL.PROPERTIES.IS_METADATA_LOADED)) {
        return;
      }
      this.setAppUnBusy();
      ...
      this._oNewLongText = m.createEntry(this._oDataHelper.getPathForLongTextSet(), {
        groupId: C.GENERAL.NOTIFICATION_BATCH_GROUP,
        changeSetId: C.GENERAL.NOTIFICATION_CHANGE_SET
      });
      ...
      if (this._oAppModel.getProperty(C.MODEL.APP_MODEL.PROPERTIES.IS_STARTUP_CREATE) === true) {
        var c = this.getOwnerComponent().getComponentData();
        if (c && c.startupParameters) {
          for (var k in c.startupParameters) {
            var v = c.startupParameters[k][0] ? c.startupParameters[k][0] : "";
            switch (k) {
      ...
      this._sNotificationId = this._sAttachObjectKey = a = this._getObjectKey(n.sPath);
      this._setObjectPaths(n.sPath.substring(1), this._oNewLongText.sPath.substring(1));
      this.refreshViewProperties();
      if (this._oAppModel.getProperty(C.MODEL.APP_MODEL.PROPERTIES.ATTACHMENTS_AVAILABLE)) {
        m.setProperty(n.sPath + "/AttachmentId", a);
        if (this._sAttachObjectKey) {
          var t = this;
          this._initAttachmentComponent(this._oAttachmentComponent, {
            mode: C.ATTACHMENTS.ATTACHMENTMODE_CREATE,
            objectType: C.ATTACHMENTS.OBJECTTYPE_NOTIFICATION,
            objectKey: this._sAttachObjectKey
          }, "pmNotifContainerAttachments").then(function (A) {
    ...
    routePatternMatched: function (e) {
      ...
      switch (E.name) {
        case C.ROUTES.CREATE:
          ...
          break;
        case C.ROUTES.EDIT:
          this._oAppModel.setProperty(C.MODEL.APP_MODEL.PROPERTIES.IS_CREATE_MODE, false);
          ...
          if (!this.getView().byId("pmNotifLongTextEdit").isBound("content")) {
            var c = new sap.ui.layout.VerticalLayout({
              width: "100%",
              content: [new i2d.eam.pmnotification.create.s1.util.ExpandableTextArea({
                editable: false,
                rows: 5,
                value: "{ReadOnlyText}",
                ...
                wrapping: "None",
                width: "100%"
              }), new i2d.eam.pmnotification.create.s1.util.ExpandableTextArea({
          ...
          this.byId("pmNotifLongTextEdit").getBinding("content").attachEventOnce("dataReceived", function () {
            this._isLongtextBindingDone = true;
            this._applyAppState();
          }, this);
          this._sAttachObjectKey = String("000000000000" + this._sNotificationId).slice(-12);
          if (this._oAppModel.getProperty(C.MODEL.APP_MODEL.PROPERTIES.ATTACHMENTS_AVAILABLE) && this._sAttachObjectKey) {
            var t = this;
            this._initAttachmentComponent(this._oAttachmentComponent, {
              mode: C.ATTACHMENTS.ATTACHMENTMODE_CREATE,
              objectType: C.ATTACHMENTS.OBJECTTYPE_NOTIFICATION,
    ...
    onShowEmployeeDetails: function (e) {
      ...
      this.getUtil().launchPopoverEmployeeQuickView(n.ReporterUserId, e.getSource(), this.getView());
    },
    ...
    onTechnicalObjectChange: function () {
      ...
      if (n.TechnicalObjectNumber !== "" && n.TechnicalObjectType !== "") {
        ...
        this.getODataHelper().countOpenNotifications(n, this._oViewProperties, this.getView().getModel());
      }
    },
    ...
    onReporterInvalidated: function () {
      this._oViewProperties.setProperty(C.MODEL.VIEW_PROP.PROPERTIES.GENERAL.VALID_USER_GIVEN, false);
    },
    ...
    onPressTechnicalObjectOverview: function (e) {
      var n = this.getObject();
      this.getUtil().launchPopoverTechnicalObjectOverview(n.TechnicalObjectNumber, n.TecObjNoLeadingZeros, n.TechnicalObjectType, e.getSource(), this.getView());
    },
    onUseTemplates: function (e) {
      var l = (this._oNewLongText ? this._oNewLongText.getPath() : this.byId("pmNotifLongTextEdit").getBinding("content").getContexts()[0].getPath()) + "/UpdateText";
    ...
    onSearchViaBarcode: function () {
      if (!this.isFunctionAvailable("cordova.plugins.barcodeScanner")) {
        ...
        sap.m.MessageToast.show(this.getResourceBundle().getText("ymsg.barcodeNotAvailable"), {
      ...
      var S = function () {
        jQuery.sap.require("sap.m.MessageToast");
        sap.m.MessageToast.show(this.getResourceBundle().getText("ymsg.barcodeScanFailed"), {
          duration: C.GENERAL.MESSAGETOAST_DELAY,
          closeOnBrowserNavigation: C.GENERAL.MESSAGETOAST_CLOSE_ON_NAV
        });
      };
    ...
    validateNewNotification: function () {
      var o = !this._checkAndMarkEmptyMandatoryFields(true) && !this._fieldWithErrorState() && (this.getModel(C.MODEL.VIEW_PROP.NAME).getProperty(C.MODEL.VIEW_PROP.PROPERTIES.GENERAL.TECHNICAL_OBJECT_VALID) || this.byId("pmNotifInputTechnicalObject").getValue() === "");
      return o;
    },
    resetValueStates: function () {
      jQuery.each($('#' + this.getView().getId()).find("*"), function (i, a) {
        if ($(a).control) {
          var c = $(a).control(0);
        }
    ...
    saveNotification: function () {
        ...
        if (d.__batchResponses) {
          ...
          for (i = 0; i < d.__batchResponses.length && !l; i++) {
            if (d.__batchResponses[i].__changeResponses) {
              var j;
              for (j = 0; j < d.__batchResponses[i].__changeResponses.length && !l; j++) {
                if (d.__batchResponses[i].__changeResponses[j].statusCode && d.__batchResponses[i].__changeResponses[j].statusCode.startsWith("2")) {
      ...
      if (this.getModel().hasPendingChanges()) {
        ...
        this._oAttachmentComponent.getApplicationState(function (r) {
          if (r) {
            this._isSubmittingChanges = true;
            try {
    ...
    cancelNotification: function () {
      ...
      if (this._isDirty() || this._oAttachmentComponent) {
        (this._oAttachmentComponent ? this._oAttachmentComponent.getApplicationState.bind(this._oAttachmentComponent) : function (c) {
          c(true);
        })(function (r) {
          if (r || this._isDirty()) {
            var c = !!this.getView().$().closest(".sapUiSizeCompact").length;
            M.show(this.getResourceBundle().getText("ymsg.warningConfirm"), {
              icon: M.Icon.WARNING,
              ...
              onClose: function (o) {
                if (o === M.Action.OK) {
                  ...
                  t._oDataHelper.cancelNotification(n, t._oAttachmentComponent, A, a);
                }
              },
            ...
            this.resetValueStates();
            this._isSubmittingChanges = true;
            this._oDataHelper.cancelNotification(n, this._oAttachmentComponent, A);
          }
        }.bind(this));
      } else {
        this.resetValueStates();
    ...
    _setPropertyTechnicalObjectValid: function (v) {
      this.getModel(C.MODEL.VIEW_PROP.NAME).setProperty(C.MODEL.VIEW_PROP.PROPERTIES.GENERAL.TECHNICAL_OBJECT_VALID, v);
      if (v) {
    ...
    _setUserCanBeNotified: function () {
      ...
      if (this.getObject().NotificationType !== "" && typeof this.getObject().NotificationType !== "undefined") {
        if (this._isInCreateMode()) {
          p = this._oDataHelper.getPathForNotificationType(this.getObject().NotificationType) + "/UserCanBeNotified";
        } else {
          p = this._oDataHelper.getPathForNotificationTypeChange(this._sLastNotificationType, this.getObject().NotificationType) + "/UserCanBeNotified";
        }
        u = this.getModel().getProperty(p) ? this.getModel().getProperty(p) : false;
      }
      this.getModel().setProperty(this.getView().getBindingContext().getPath() + "/UserCanBeNotified", u);
      if (!u) {
    ...
    _setNotificationTypeText: function () {
      ...
      if (this.getObject().NotificationType !== "" && typeof this.getObject().NotificationType !== "undefined") {
        if (this._isInCreateMode()) {
          p = this._oDataHelper.getPathForNotificationType(this.getObject().NotificationType) + "/Description";
        } else {
          p = this._oDataHelper.getPathForNotificationTypeChange(this._sLastNotificationType, this.getObject().NotificationType) + "/TargetNotificationTypeName";
        }
        n = this.getModel().getProperty(p) ? this.getModel().getProperty(p) : "";
      }
      this.getModel().setProperty(this.getView().getBindingContext().getPath() + "/NotificationTypeText", n);
    },
        ...
        this._oNotifTypeControl.onAfterRendering = function () {
          ...
          if (c) {
            ...
            c.keydown(function (e) {
              if (!(e.keyCode === jQuery.sap.KeyCodes.ARROW_DOWN || e.keyCode === jQuery.sap.KeyCodes.ARROW_LEFT || e.keyCode === jQuery.sap.KeyCodes.ARROW_RIGHT || e.keyCode === jQuery.sap.KeyCodes.ARROW_UP || e.keyCode === jQuery.sap.KeyCodes.TAB || e.keyCode === jQuery.sap.KeyCodes.F1 || e.keyCode === jQuery.sap.KeyCodes.F10 || e.keyCode === jQuery.sap.KeyCodes.F11 || e.keyCode === jQuery.sap.KeyCodes.F12 || e.keyCode === jQuery.sap.KeyCodes.F2 || e.keyCode === jQuery.sap.KeyCodes.F3 || e.keyCode === jQuery.sap.KeyCodes.F4 || e.keyCode === jQuery.sap.KeyCodes.F5 || e.keyCode === jQuery.sap.KeyCodes.F6 || e.keyCode === jQuery.sap.KeyCodes.F7 || e.keyCode === jQuery.sap.KeyCodes.F8 || e.keyCode === jQuery.sap.KeyCodes.F9 || e.keyCode === jQuery.sap.KeyCodes.ESCAPE || e.keyCode === jQuery.sap.KeyCodes.ENTER)) {
                e.preventDefault();
                return false;
              }
            });
    ...
    _bindNotificationTypeControl: function () {
        ...
        p = new sap.ui.model.Filter({
          ...
          test: function (s) {
            if (t._isInCreateMode() || s === c) {
              return true;
            } else if (t.getModel().getProperty("/NotificationTypeChangeSet(SourceNotificationType='" + c + "',TargetNotificationType='" + s + "')")) {
              return true;
            }
            return false;
          }
        ...
        events: {
          dataReceived: function () {
            if (!t._isInCreateMode() && t.getObject()) {
              if (!t.getModel().getProperty(t.getODataHelper().getPathForNotificationType(t.getObject().NotificationType))) {
                t._oNotifTypeControl.addItem(new sap.ui.core.ListItem({
                  key: t.getObject().NotificationType,
                  text: t.getObject().NotificationType + ' (' + t.getObject().NotificationTypeText + ')'
                }));
              }
            }
          }
      ...
      if (!this._isInCreateMode()) {
        if (jQuery.grep(Object.keys(this.getModel().getProperty("/")), function (k) {
    ...
    _afterSuccessSave: function () {
      ...
      var c = !!this.getView().$().closest(".sapUiSizeCompact").length;
      M.show(this.getResourceBundle().getText("ymsg.saveCreateText", this.getObject().NotificationNumber), {
        ...
        onClose: function (a) {
          ...
          if (a === M.Action.OK) {
            ...
            jQuery.sap.require("sap.m.MessageToast");
            sap.m.MessageToast.show(t.getResourceBundle().getText("ymsg.saveCreateText", t.getObject().NotificationNumber), {
              duration: C.GENERAL.MESSAGETOAST_DELAY,
              closeOnBrowserNavigation: C.GENERAL.MESSAGETOAST_CLOSE_ON_NAV
            });
          }
    ...
    _isDesktop: function () {
      return this.getModel(C.MODEL.DEVICE_MODEL.NAME).getProperty(C.MODEL.DEVICE_MODEL.PROPERTIES.IS_DESKTOP);
    },
    _isInCreateMode: function () {
      return this._oAppModel.getProperty(C.MODEL.APP_MODEL.PROPERTIES.IS_CREATE_MODE);
    },
    _checkAndMarkEmptyMandatoryFields: function (e) {
      ...
      jQuery.each(this._getMandatoryFields(), function (i, a) {
        if (a && a.getValue) {
          ...
          if (!a.getSelectedItem() || a.getSelectedItem().getText().trim() === "") {
            E = true;
            if (e) {}
          }
        }
    ...
    _fieldWithErrorState: function () {
      ...
      jQuery.each($('#' + this.getView().getId()).find("*"), function (i, a) {
        ...
        if (c && c.getValueState && c.getValueState() === sap.ui.core.ValueState.Error) {
          e = true;
        }
      });
      return e;
    ...
    _getMandatoryFields: function () {
      ...
      if (this._oAppModel.getProperty(C.MODEL.APP_MODEL.PROPERTIES.IS_CREATE_MODE)) {
    ...
    isLinkAdditionalSearchOptionsVisible: function () {
      if (this.extHookAdditionalSearchOptions) {
        ...
        return this.getModel(C.MODEL.APP_MODEL.NAME).getProperty(C.MODEL.APP_MODEL.PROPERTIES.BARCODE_AVAILABLE);
      }
    },
      ...
      p.done(function (a, u, n) {
        if (n !== C.ROUTES.NavType.initial) {
          ...
          if (A !== t._sAttachObjectKey) {
            t._initAttachmentComponent(t._oAttachmentComponent, {
              mode: C.ATTACHMENTS.ATTACHMENTMODE_CREATE,
              objectType: C.ATTACHMENTS.OBJECTTYPE_NOTIFICATION,
              objectKey: this._sAttachObjectKey
            }, "pmNotifContainerAttachments").then(function (o) {
    ...
    getCurrentAppState: function () {
      var l = {};
      if (this._oAppModel.getProperty(C.MODEL.APP_MODEL.PROPERTIES.IS_CREATE_MODE)) {
        l = this.getView().byId("pmNotifLongTextCreate").getBindingContext().getObject();
      } else {
        l = this.getView().byId("pmNotifLongTextEdit").getContent()[0].getBindingContext().getObject();
      }
      return {
    ...
    _messageSkipPendingChanges: function (F) {
      var t = this;
      var c = !!this.getView().$().closest(".sapUiSizeCompact").length;
      M.show(this.getResourceBundle().getText("ymsg.messageWarningSkipPendingChanges"), {
        icon: M.Icon.WARNING,
        title: this.getResourceBundle().getText("xsel.messageSeverityWarning"),
        actions: [M.Action.OK, M.Action.CANCEL],
        onClose: function (a) {
    ...
    _restoreAppStateData: function (d) {
      var c = this._oAppModel.getProperty(C.MODEL.APP_MODEL.PROPERTIES.IS_CREATE_MODE);
      var p = "";
      for (var o in d) {
        var e = d[o];
        if (o.startsWith("notificationData")) {
          ...
          for (var a in e) {
            switch (a) {
              ...
              case "NotificationType":
                ...
                s.getBinding("items").attachEventOnce("change", function () {
          ...
          if (c) {
            ...
            p = this.byId("pmNotifLongTextEdit").getBinding("content").getContexts()[0].getPath() + "/";
          }
